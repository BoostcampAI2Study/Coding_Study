...
loc_list, piece_list = [], []
for piece in pieces:
    loc_list.append((piece[0] - 1, piece[1] - 1))   # x, y
    piece_list.append((piece[2], 1))                # 방향, 쌓여진 순서

cnt = 0
while cnt <= 1000:
    cnt += 1  # count turn
    for i in range(k):
        direct, rank = piece_list[i]    # 방향, 쌓여진 순서
        x, y = loc_list[i]              # x, y
        nx, ny = x + dir_dict[direct][0], y + dir_dict[direct][1]   # 말 이동
        ...
        if board[nx][ny] == 0:
            for j in range(k):
                if loc_list[j] == (x, y) and piece_list[j][1] >= rank:
                    loc_list[j] = (nx, ny)
                    piece_list[j] = (piece_list[j][0], piece_list[j][1] - rank + loc_cnt + 1)
        # 빨강
        elif board[nx][ny] == 1:
